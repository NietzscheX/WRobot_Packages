...
try
{
    string onlineFile = "https://bambowrobot.000webhostapp.com/vanilla/GoldMakingPluginVanilla.cs";
    ...
    if (!string.IsNullOrWhiteSpace(currentFileContent) && !string.IsNullOrWhiteSpace(onlineFileContent))
    {
        if (currentFileContent.Length == 39 && onlineFileContent.Length == 39) // 39 is size of encrypted files with option "Short file"
        {
            ...
            {
                robotManager.Helpful.Logging.Write("[Bambo_GMPVanilla_Plugin]: New version found, try to update.");
            ...
            if (currentFileContent == onlineFileContent)
                robotManager.Helpful.Logging.Write("[Bambo_GMPVanilla_Plugin]: The version on your pc is the latest updated version.");
        }
        else
            robotManager.Helpful.Logging.Write("[Bambo_GMPVanilla_Plugin]: Your currently running version is not encrypted.");
    }
}
